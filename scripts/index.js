...
// src/range/FiniteRange.ts
class FiniteRange extends NumberRange {
  constructor(range) {
...
// src/range/IntRange.ts
class IntRange extends FiniteRange {
  constructor(range) {
...
class ArrayModel extends TypeModel {
  ...
  test(x) {
    return Array.isArray(x) && x.every((e) => this.type.test(e));
  }
  withLength(range) {
    const min = range.min === undefined ? 0 : range.min;
    const max = range.max === undefined ? Infinity : range.max;
...
class UndefindableModel extends TypeModel {
  ...
  test(x) {
    return this.type.test(x) || sentry.undefined.test(x);
  }
  unwrap() {
    return this.type;
  }
...
class NullableModel extends TypeModel {
  ...
  test(x) {
    return this.type.test(x) || sentry.null.test(x);
  }
  unwrap() {
    return this.type;
  }
...
class LiteralModel extends PrimitiveModel {
  ...
  toString() {
    switch (typeof this.value) {
      ...
      case "symbol":
        return "symbol(" + (this.value.description === undefined ? "" : '"' + this.value.description + '"') + ")";
    }
  }
...
var SYMBOL_FOR_PRIVATE_CONSTRUCTOR = Symbol();
...
var sentry = class extends TypeSentry {
  static INSTANCE = new this(SYMBOL_FOR_PRIVATE_CONSTRUCTOR);
}.INSTANCE;
var typeSentry = sentry;

// src/vector/DualAxisRotationBuilder.ts
class DualAxisRotationBuilder {
  __yaw__;
  __pitch__;
  ...
  clamp(min, max) {
    return this.operate(min, max, (val, min2, max2) => {
  ...
  format(format, digits) {
    ...
    return format.replace(/\$yaw/g, cx).replace(/\$pitch/g, cy).replace("$c", cx).replace("$c", cy).replace(/\$c/g, "");
  }
  toString() {
...
class Vector3Builder {
  ...
  getDirectionTo(other) {
    return Vector3Builder.from(other).clone().subtract(this).normalize();
  }
  project(other) {
  ...
  format(format, digits) {
    ...
    return format.replace(/\$x/g, cx).replace(/\$y/g, cy).replace(/\$z/g, cz).replace("$c", cx).replace("$c", cy).replace("$c", cz).replace(/\$c/g, "");
  }
  toString() {
  ...
  static from(arg0, arg1 = 0) {
    if (this.isVector3(arg0)) {
      ...
      switch (arg0) {
        case "Up" /* Up */:
          return Vector3Builder.up();
        case "Down" /* Down */:
          return Vector3Builder.down();
        case "North" /* North */:
          return Vector3Builder.back();
        case "South" /* South */:
          return Vector3Builder.forward();
        case "East" /* East */:
          return Vector3Builder.left();
        case "West" /* West */:
          return Vector3Builder.right();
        default:
          throw new TypeError("Unknown Direction Value");
      }
    } else {
  ...
  static min(a, b) {
    return this.from(a).clone().operate(b, (a2, b2) => Math.min(a2, b2));
  }
  static max(a, b) {
    return this.from(a).clone().operate(b, (a2, b2) => Math.max(a2, b2));
  }
}

// src/noise/CubeLattice.ts
class CubeLattice {
  static GRADIENT_VECTORS = [
    { x: 1, y: 1, z: 0 },
    { x: -1, y: 1, z: 0 },
    { x: 1, y: -1, z: 0 },
    { x: -1, y: -1, z: 0 },
  ...
  getGradientVectorByHash(hash) {
    return CubeLattice.GRADIENT_VECTORS[hash & 15];
  }
  getGradientVectors(permutation) {
...
class PerlinNoise {
  ...
  noise3(v, options) {
    const vector = Vector3Builder.from(v).scale(options.frequency).add(this.offset);
...
// src/index.ts
var noise = new PerlinNoise(new Xorshift32(7));
var options = {
  amplitude: 10,
  frequency: 9
};
